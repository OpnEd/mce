<x-filament-panels::page>
    ...
    @php
        ...
        $group8 = $entries->filter(fn ($entry) => str_starts_with((string) ($entry['entry_id'] ?? ''), '8.'))->values();

        $sectionDescription = minutesIvcSectionDescriptionFromConfig(
            ['Revisión de productos', 'revision-productos', 8],
            'Sin descripción normativa disponible.'
        );
    @endphp

    <x-filament::section>
        <x-slot name="heading">8. Revisión de productos</x-slot>
        <x-slot name="description">{{ $sectionDescription }}</x-slot>
    </x-filament::section>

    <x-settings.group-wrapper title="8. Revisión de productos">
        @forelse ($group8 as $entry)
