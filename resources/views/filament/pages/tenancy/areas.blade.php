<x-filament-panels::page>
    ...
    @php
        ...
        $group5 = $entries->filter(fn ($entry) => str_starts_with((string) ($entry['entry_id'] ?? ''), '5.'))->values();
    ...
    <x-filament::section>
        <x-slot name="heading">5. Áreas</x-slot>
        <x-slot name="description">{{ $sectionDescription }}</x-slot>
    </x-filament::section>

    <x-settings.group-wrapper title="5. Áreas">
        @forelse ($group5 as $entry)
