<head>
    <title>Sale to {{ $invoice->sale->customer?->name }}</title>
</head>
<div
    ...
    <table style="width: 100%; border-bottom: 1px solid #e5e7eb; padding-bottom: 16px;">
        <tr>
            <td>
                ...
                <p style="color: #a0aec0;">Invoice #{{ $invoice->factura?->code ?? $invoice->id }}</p>
                <p style="color: #a0aec0;">Date: {{ $invoice->created_at->format('Y-m-d') }}</p>
            </td>
    ...
    <table style="width: 100%; margin-top: 24px;">
        <tr>
            <td>
                ...
                <p style="color: #718096;">{{ $invoice->sale?->customer?->name }}</p>
                <p style="color: #718096;">{{ $invoice->sale?->customer?->address }}</p>
                <p style="color: #718096;">Email: {{ $invoice->sale?->customer?->email }}</p>
            </td>
            <td style="text-align: right;">
        ...
        <tbody>
            @foreach ($invoice->items as $index => $item)
                <tr>
                    ...
                    <td style="border: 1px solid #e2e8f0; padding: 8px; color: #718096;">
                        {{ $item->sale_item?->inventory?->product?->name }}
                    </td>
        ...
        <tfoot>
            ...
            <tr>
                <td colspan="4"
                    ...
                    Total</td>
                <td style="border: 1px solid #e2e8f0; padding: 8px; text-align: right; color: #4a5568;">
                    ${{ $invoice->amount - $invoice->discount + $invoice->iva }}</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div style="margin-top: 24px; text-align: center; color: #a0aec0;">
        ...
        <p>{{ __('If you have any questions about this invoice, please contact us at :email', ['email' => $invoice->team?->email]) }}
        </p>
    </div>
</div>
