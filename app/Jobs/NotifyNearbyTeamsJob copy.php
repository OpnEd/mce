        ...
        foreach ($newNotifications as $item) {
            ...
            foreach ($team->users as $user) {
                ...
                try {
                    ...
                    // 1) Guardar notificación en DB / Mail
                    /* $notification = new NewExternalOrderNotification($order, $team, (int) round($item['distance_m']));
                    $notification->id = Str::uuid()->toString(); */

                    /* Log::debug('NotifyNearbyTeamsJob: Insertando notificación manual', [
                        'notification_id' => $notification->id,
                        'team_id'         => $team->id,
                        'user_id'         => $user->id,
                    ]); */

                    // Insertamos manualmente para asegurar que el 'team_id' se guarde en la columna correspondiente
                    /* DB::table('notifications')->insert([
                        ...
                        'team_id'         => $team->id,
                        'data'            => json_encode($notification->toDatabase($user)),
                        'read_at'         => null,
                        'created_at'      => now(),
                        'updated_at'      => now(),
                    ]); */
                    ...
                    // 3) Mostrar notificación en Filament (UI toast)
                    /* FilamentNotification::make()
                        ->title('Nuevo pedido disponible')
                        ->body("Orden {$order->external_order_id} a " . round($item['distance_m']) . " m.")
                        ->success()
                        ->send($user); */
