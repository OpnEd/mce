    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ->where(function (Builder $query) {
                ...
                $query->whereNull('team_id') // Cursos Globales
                    ->orWhere('team_id', $tenantId); // Cursos Propios
            })
            ->with(['instructor'])
        ...
        return $table
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                    // Acción: Inscribirme (Solo para cursos Globales no inscritos)
                    Tables\Actions\Action::make('enroll')
                        ...
                        ->hidden(
                            fn($record) =>
                            $record->team_id !== null || // Si es propio, ya está "inscrito"
                                $record->teams()->where('teams.id', Filament::getTenant()->id)->exists()
                        )
                        ->action(fn($record) => $record->teams()->attach(Filament::getTenant()->id)),

                    // Acciones estándar (Solo para cursos propios)
